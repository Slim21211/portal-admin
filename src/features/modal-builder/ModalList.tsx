...
export function ModalList() {
  ...
  const [setActive, { isLoading: isSettingActive, error: activeError }] =
    useSetActiveModalMutation();
  ...
  if (!modals?.length)
    return (
      <p className={styles.status}>Модалок пока нет — создай первую выше.</p>
    );
...

  {
    activeError && (
      <p className={styles.statusError}>
        Ошибка:{' '}
        {typeof activeError === 'string' ? activeError : 'Не удалось обновить'}
      </p>
    );
  }
  ...
  return (
    <div className={styles.root}>
      ...
      <p className={styles.subtitle}>
        Только одна может быть активной — именно её увидят сотрудники на
        портале.
      </p>
      ...
      <ul className={styles.list}>
        {modals.map((modal) => (
          <li
            key={modal.id}
            className={`${styles.item} ${modal.is_active ? styles.itemActive : ''}`}
          >
            <div className={styles.itemInfo}>
              <div className={styles.itemHeader}>
                <span className={styles.itemName}>
                  {modal.name || 'Без названия'}
                </span>
                {modal.is_active && (
